...
import {
  getCurrentAccountFromRequestOrNull,
...
export const getCurrentCometChatProfileOrNullServerFn = createServerFn({ method: 'GET' }).handler(
  async (): Promise<CurrentCometChatProfile | null> => {
    const account = await getCurrentAccountFromRequestOrNull(getRequest())
