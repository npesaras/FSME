...
type ScopeDocumentTrackingStatus = 'Accepted' | 'Pending' | 'Rejected'

interface ScopeDocumentTrackingRecord {
  id: string
  date: string
  status: ScopeDocumentTrackingStatus
  fileName: string
  remarks: string
}

const columns: ColumnDef<ScopeDocumentTrackingRecord>[] = [
  {
    accessorKey: 'date',
    header: () => (
...
interface ScopeDocumentTrackingTableProps {
  data: ScopeDocumentTrackingRecord[]
}

export function ScopeDocumentTrackingTable({
  data,
}: ScopeDocumentTrackingTableProps) {
  return (
