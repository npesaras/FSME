...
export const appwriteTableSchemas = {
  ...
  cometUserProfiles: {
    id: 'comet_user_profiles',
    name: 'Comet User Profiles',
    status: 'live',
    description:
      'Mirrored CometChat identity records linked to Appwrite-authenticated users for chat bootstrap.',
    columns: [
      {
        key: 'user_id',
        kind: 'varchar',
        required: true,
        size: 255,
        description: 'Appwrite Auth user ID for the account that owns this CometChat identity.',
      },
      {
        key: 'cometchat_uid',
        kind: 'varchar',
        required: true,
        size: 255,
        description: 'Stable CometChat UID used to log the account into chat.',
      },
      {
        key: 'full_name',
        kind: 'varchar',
        required: true,
        size: 255,
        description: 'Display name mirrored to the CometChat user profile.',
      },
      {
        key: 'role',
        kind: 'enum',
        required: true,
        elements: appRoles,
        description: 'Application role mirrored for troubleshooting and future chat rules.',
      },
      {
        key: 'avatar_url',
        kind: 'varchar',
        required: false,
        size: 2048,
        description: 'Optional avatar URL mirrored to CometChat when available.',
      },
      {
        key: 'profile_link',
        kind: 'varchar',
        required: false,
        size: 2048,
        description: 'Optional app profile link associated with the CometChat user.',
      },
      {
        key: 'auth_token',
        kind: 'varchar',
        required: false,
        size: 2048,
        description: 'Latest known CometChat auth token captured during provisioning when available.',
      },
    ],
    indexes: [
      { key: 'comet_user_profiles_user_id', type: 'unique', columns: ['user_id'] },
      { key: 'comet_user_profiles_uid', type: 'unique', columns: ['cometchat_uid'] },
      { key: 'comet_user_profiles_role', type: 'key', columns: ['role'], orders: ['asc'] },
    ],
  },
  applications: {
    id: 'applications',
