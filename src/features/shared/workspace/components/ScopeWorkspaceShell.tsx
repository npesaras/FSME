...
import type { ChatActor } from '@/features/shared/chat/config'
import { preloadChatWorkspace } from '@/features/shared/chat/preload'
import type {
  ScopeBasePath,
  ScopeSidebarRenderProps,
  ScopeWorkspaceSection,
} from '../types'

interface ScopeWorkspaceShellProps {
  account: AuthAccount
  activeSection: ScopeWorkspaceSection
  actor: ChatActor
  children: ReactNode
  emptyInitials: string
  homePath: ScopeBasePath
  renderSidebar: (props: ScopeSidebarRenderProps) => ReactNode
  roleLabel: string
}

function getInitials(name: string, emptyInitials: string) {
  const parts = name
    .trim()
    .split(/\s+/)
    .filter(Boolean)
...
export function ScopeWorkspaceShell({
  account,
  activeSection,
  actor,
  children,
  ...
  useEffect(() => {
    ...
    const warmChatWorkspace = () => {
      ...
      void preloadChatWorkspace(actor)
    }
            ...
            <div className="relative">
              ...
              >
                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-border bg-primary/10 text-sm font-bold text-primary shadow-sm">
                  {getInitials(account.name, emptyInitials)}
                </div>
